...
const AddModal = props => {
  ...
  const handleOk = () => {
    form.validateFields().then((values) => {
        ipcRenderer.send('add-data', {...values});
        ipcRenderer.on('add-data', (event, data) => {
  ...
  return (
      <Modal title="新增" open onOk={handleOk} onCancel={handleCancel}>
        <Form form={form} className='add-modal-form'>
            <Form.Item
                name="名称"
                label="名称"
                colon={false}
